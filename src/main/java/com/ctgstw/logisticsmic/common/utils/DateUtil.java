...
public class DateUtil {
     ...
     * @throws
     * @Title: compareTime
     * @Description: TODO(日期时间比较 ， 如果s > = e 返回true 否则返回false)
     */
...
//		return src.substring(0, 4) + src.substring(4, 6) + src.substring(6);
    ...
    public static void main(String args[]) {
        ...
        //解析日期
        Instant s = Instant.now();//获取的是美国时间
        OffsetDateTime time = s.atOffset(ZoneOffset.ofHours(8));//我们是东8区 加8小时

        System.out.println(time);

        String dateStr = "2016-05-02 08:20:20";
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
        LocalDateTime date = LocalDateTime.parse(dateStr, formatter);
        LocalDateTime dateInstant = LocalDateTime.ofInstant(s, ZoneId.systemDefault());

        System.out.println(dateInstant.format(formatter));
